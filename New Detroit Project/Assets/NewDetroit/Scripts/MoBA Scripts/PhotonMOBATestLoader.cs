...
public class PhotonMOBATestLoader : MonoBehaviour
{
    ...
    public void Awake()
    {
        ...
        else
        {
            if (joinedId == 0)
            {
				GameObject heroInst = PhotonNetwork.Instantiate("Skelterbot", Camera.main.transform.position, new Quaternion(), 0);
            ...
            {
				GameObject heroInst = PhotonNetwork.Instantiate("Rob Render", Camera.main.transform.position, new Quaternion(), 0);
				heroInst.GetComponent<ThirdPersonCamera>().cameraTransform = Camera.main.transform;
