...
/////// Normals:
                ...
                float3 normalLocal = UnpackNormal(tex2D(_Normal,TRANSFORM_TEX(node_259.rg, _Normal))).rgb;
...
////// Emissive:
                float4 node_138 = tex2D(_Diffuse,TRANSFORM_TEX(node_259.rg, _Diffuse));
                float node_163 = (pow(node_138.a,_PowerIntensity)*_MultiplyIntensity);
                float3 emissive = (((node_163*_Color.rgb)*tex2D(_Mask,TRANSFORM_TEX(node_259.rg, _Mask)).rgb)*_Emission);
///////// Gloss:
...
/// Final Color:
        ...
        Pass {
            ...
            fixed4 frag(VertexOutput i) : COLOR {
                ...
                float3 viewDirection = normalize(_WorldSpaceCameraPos.xyz - i.posWorld.xyz);
/////// Normals:
                float2 node_260 = i.uv0;
                float3 normalLocal = UnpackNormal(tex2D(_Normal,TRANSFORM_TEX(node_260.rg, _Normal))).rgb;
                float3 normalDirection =  normalize(mul( normalLocal, tangentTransform )); // Perturbed normals
...
////// Specular:
                NdotL = max(0.0, NdotL);
                float4 node_138 = tex2D(_Diffuse,TRANSFORM_TEX(node_260.rg, _Diffuse));
                float node_163 = (pow(node_138.a,_PowerIntensity)*_MultiplyIntensity);
                float3 specularColor = float3(node_163,node_163,node_163);
