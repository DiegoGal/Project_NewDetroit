    ...
    SubShader {
        ...
        Pass {
            ...
            fixed4 frag(VertexOutput i) : COLOR {
                ...
                float3 normalLocal = UnpackNormal(tex2D(_normal,TRANSFORM_TEX(node_42.rg, _normal))).rgb;
...
////// Specular:
                NdotL = max(0.0, NdotL);
                float4 node_2 = tex2D(_diffusespecula,TRANSFORM_TEX(node_42.rg, _diffusespecula));
...
/// Final Color:
        ...
        Pass {
            ...
            fixed4 frag(VertexOutput i) : COLOR {
                ...
                float3 normalLocal = UnpackNormal(tex2D(_normal,TRANSFORM_TEX(node_43.rg, _normal))).rgb;
                float3 normalDirection = normalize( mul( normalLocal, tangentTransform ) );
...
////// Specular:
                NdotL = max(0.0, NdotL);
                float4 node_2 = tex2D(_diffusespecula,TRANSFORM_TEX(node_43.rg, _diffusespecula));
                float3 specularColor = (node_2.a*_SpecularColor.rgb);
