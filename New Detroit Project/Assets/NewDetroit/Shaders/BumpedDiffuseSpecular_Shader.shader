...
/////// Normals:
                ...
                float3 normalLocal = UnpackNormal(tex2D(_Normal,TRANSFORM_TEX(node_253.rg, _Normal))).rgb;
...
////// Specular:
                NdotL = max(0.0, NdotL);
                float4 node_138 = tex2D(_Diffuse,TRANSFORM_TEX(node_253.rg, _Diffuse));
...
/////// Normals:
                ...
                float3 normalLocal = UnpackNormal(tex2D(_Normal,TRANSFORM_TEX(node_254.rg, _Normal))).rgb;
                float3 normalDirection =  normalize(mul( normalLocal, tangentTransform )); // Perturbed normals
...
////// Specular:
                NdotL = max(0.0, NdotL);
                float4 node_138 = tex2D(_Diffuse,TRANSFORM_TEX(node_254.rg, _Diffuse));
                float node_163 = (pow(node_138.a,_PowerIntensity)*_MultiplyIntensity);
                float3 specularColor = float3(node_163,node_163,node_163);
